...
const Hero = () => {
  ...
  const cities = useMemo(() => {
    ...
    return [...new Set(rooms.map((room) => room.hotel.city))];
  }, [rooms]);
      ...
      <div className="flex items-center   w-full h-screen">
        <div className="flex flex-col items-start space-y-3 pt-30">
          ...
          <div className="flex flex-wrap items-center gap-3 mb-1">
            ...
            <Link
              to="/rooms"
              className="px-5 py-2.5 rounded-full bg-white/5 border border-white/20 backdrop-blur-md text-white text-sm font-medium hover:bg-white/15 transition-all duration-300 inline-flex"
            >
              Explore Rooms
            </Link>
          </div>
